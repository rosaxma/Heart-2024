...
varList <- read.table(opt$variantList, header=T, sep="\t", stringsAsFactors=F) %>% select(CredibleSet, Coding, SpliceSite) %>% filter(Coding | SpliceSite) %>% distinct()

e2g <- e2g %>% select(chr, start, end, CredibleSet, rsid, distance, !!sym(opt$scoreCol), CellType, TargetGene, Trait) %>% rename(enh_chr=chr, enh_start=start, enh_end=end, Celltype=CellType) %>% distinct() %>% group_by(enh_chr, enh_start, enh_end,  CredibleSet, Celltype) %>% mutate(TargetGeneRank=dense_rank(-!!sym(opt$scoreCol))) %>% ungroup() %>% filter(TargetGeneRank<=opt$E2GFilter) %>% left_join(varList) %>% rename(VariantInCSOverlapCoding=Coding,VariantInCSOverlapSpliceSite=SpliceSite)
print(colnames(e2g)) 
...
enh_df_gep <- enh_df_gep %>% inner_join(gene_beta_metadata, by=c("TargetGene"="Gene", "GEP")) %>% left_join(gep_tf ) %>% select(CredibleSet, Celltype, TargetGene, GEP,GeneRankInGEP, Proportion_in_GEP, TFs)%>% group_by(CredibleSet, Celltype, TargetGene) %>% arrange(desc(Proportion_in_GEP)) %>% mutate(GEP=paste(unique(GEP), collapse="|")) %>% mutate(GeneRankInGEP=paste(unique(GeneRankInGEP),collapse="|")) %>% mutate(Proportion_in_GEP=paste(unique(Proportion_in_GEP), collapse="|")) %>% mutate(TFs=paste(unique(TFs), collapse="|")) %>% distinct()
print(head(enh_df_gep))
enh_df <- enh_df %>% select(-c(GEP, Proportion_in_GEP)) %>% left_join(enh_df_gep)
########################################################################
ldsc_df <- read.table(opt$LDSC_enrichment, sep="\t", header=T, stringsAsFactors=F) %>% select(Category, Enrichment, Enrichment_p_fdr, Coefficient_z.score, finemapped_trait, n_LeadSNPs) %>% filter(Enrichment > 1) %>% rename(Celltype=Category, LDSC_p_fdr=Enrichment_p_fdr, LDSC_zscore=Coefficient_z.score, Trait=finemapped_trait) 

enh_df <- enh_df %>% left_join(ldsc_df)%>% distinct() %>% arrange(CredibleSet, -!!sym(opt$scoreCol))
print(head(enh_df))
########################################################################
